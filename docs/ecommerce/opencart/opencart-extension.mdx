...
1. No painel de administração da sua loja, localize a barra lateral e clique na opção "Extensions" (Extensões).

2. Em seguida, no submenu que é exibido, clique na opção "Marketplace":
...
Role um pouco a página até encontrar a seção "Download". Clique nessa seção e em seguida no botão "Download" para carregar a nossa extensão na sua loja:
...
Ao acessar a página do instalador de extensões, localize o botão de upload azul localizado no canto superior direito do seu painel administrativo. Clique nesse botão e selecione o arquivo da extensão que deseja carregar:
...
Na página de extensões, você pode usar a opção de filtro para selecionar o tipo _Payment_ e exibir apenas as extensões relacionadas a essa categoria específica:
